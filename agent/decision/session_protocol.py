...
def attach_protocol_to_session(
    ...
    *,
    ...
    session.protocol_risk_factors = [
        factor.model_dump(mode="json") for factor in protocol.risk_factors
    ]
    return protocol
...
def protocol_from_session(session: CallSessionState) -> PostOpProtocol:
    ...
    risk_factors = [
        RiskFactorDefinition.model_validate(item) for item in session.protocol_risk_factors
    ]
