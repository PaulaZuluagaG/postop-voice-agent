...
class DocumentService:
    ...
    def analyze_document(
        ...
        *,
        ...
        normalized = normalize_procedure_id(suggested)
        if not is_valid_procedure_id(normalized):
            raise ValueError(
                f"El clasificador devolvió un procedure inválido: {suggested!r}. "
                "Corrígelo manualmente en el modal."
            )
        temp_id = uuid.uuid4().hex
        self._pending_uploads[temp_id] = temp_path
        return ProcedureSuggestion(suggested_procedure=normalized, temp_id=temp_id)
    ...
    def confirm_document(
        ...
        *,
        ...
        return DocumentItem(
            ...
            procedure_type=procedure_display_label(normalized_procedure),
    ...
    def upload_document(
        ...
        *,
        ...
        return DocumentItem(
            source_id=indexed.source_id if indexed else "",
            procedure_type=procedure_display_label(normalized),
            file_name=destination.name,
            chunk_count=indexed.chunk_count if indexed else report.total_chunks,
        )
    ...
    def _to_item(source: SourceAggregate) -> DocumentItem:
        ...
        return DocumentItem(
            ...
            procedure_type=procedure_display_label(procedure_id),
